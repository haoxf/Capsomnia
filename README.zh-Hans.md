...
需要让本地任务继续运行时，请开启Caps Lock。关闭Caps Lock会解除Capsomnia自身控制；其他睡眠控制器仍可能让Mac保持唤醒。
...
- 自动关闭定时器（可选）：可选择15分钟到8小时的预设，或设置1分钟到24小时的自定义时长。到时后Capsomnia会关闭；只有禁用外部控制器兼容时才会立即请求系统睡眠。
- Caps Lock关闭：Capsomnia只解除一次防休眠。启用外部控制器兼容后，其他应用随后可以重新启用防休眠，Capsomnia不会覆盖。
- Capsomnia开启时合盖：仅在未连接外接显示器时关闭显示屏，后台任务继续运行。
- 退出应用：Capsomnia开启时会解除其防休眠；兼容模式下若Capsomnia已经关闭，则保留外部控制器的状态。
...
- 禁用兼容模式时，自动关闭定时器到时后会明确让Mac进入睡眠。请先保存工作，并设置足够长的时间以确保任务完成。
...
“合盖时关闭显示屏”、“尊重外部睡眠控制器”和“登录时启动”默认开启，不会显示在初始设置中。之后可以再次打开Capsomnia修改所有设置。兼容模式会在Capsomnia关闭时写一次`SleepDisabled=0`，之后把外部重新写入的`1`视为其他控制器持有；菜单栏保持灰色关闭状态，并在提示文字中说明外部状态。高级设置中还可以配置自动关闭定时器和全局快捷键。定时器默认关闭。每次开启Capsomnia时都会从所选时长重新开始，也可以使用重启按钮重置当前倒计时。启用“防止输入锁定为大写”时，“显示菜单栏图标”仍可独立开关。即使隐藏菜单栏图标，发生错误时也会暂时显示红色圆点。
...
通过软件包安装的应用文件、辅助程序和系统LaunchAgent均归`root:wheel`所有。软件包中的辅助程序也使用与应用相同的Developer ID签名。Capsomnia会在每次切换后以及之后每10秒检查一次实际的`SleepDisabled`状态。如果辅助程序无法应用更改、状态无法验证，或设置发生漂移，菜单栏圆点会变为红色，Capsomnia会在5秒后重试。唯一例外是启用外部控制器兼容、Capsomnia处于关闭状态而`SleepDisabled=1`：此时会接受外部值且不再清零。即使平时隐藏菜单栏图标，发生错误时也会暂时显示红色圆点。
...
禁用外部控制器兼容时，自动关闭定时器会在成功关闭Caps Lock并确认`SleepDisabled=0`后，以当前用户身份直接运行`/usr/bin/pmset sleepnow`；兼容模式会取消这一立即睡眠请求。该命令不使用`sudo`，也不会扩大辅助程序或sudoers权限。
