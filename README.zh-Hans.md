...
- 自动关闭定时器（可选）：可选择15分钟到8小时的预设、1分钟到24小时的自定义时长，或指定到点时间（例如23:00）。到时后Capsomnia会关闭；只有禁用外部控制器兼容时才会立即请求系统睡眠。
...
“合盖时关闭显示屏”、“尊重外部睡眠控制器”和“登录时启动”默认开启，不会显示在初始设置中。完成初始设置后，打开 Capsomnia 会直接进入高级设置；可用返回按钮回到精简页。兼容模式会在Capsomnia关闭时写一次`SleepDisabled=0`，之后把外部重新写入的`1`视为其他控制器持有；菜单栏保持灰色关闭状态，并在提示文字中说明外部状态。高级设置中还可以配置自动关闭定时器和全局快捷键。定时器默认关闭。每次开启Capsomnia时，时长定时器会从所选时长重新开始，到点定时器会排到下一次该钟点。也可以使用重启按钮重置当前倒计时。启用“防止输入锁定为大写”时，“显示菜单栏图标”仍可独立开关。即使隐藏菜单栏图标，发生错误时也会暂时显示红色圆点。
