...
로컬 작업을 계속 돌리고 싶을 때 Caps Lock을 켜세요. Caps Lock을 끄면 Capsomnia의 제어가 해제되지만, 다른 잠자기 제어기가 Mac을 계속 깨워 둘 수 있습니다.
...
- 자동 종료 타이머(선택 사항): 15분~8시간 프리셋, 1분~24시간 사용자 지정 시간, 또는 23:00 같은 시각을 설정할 수 있습니다. 시간이 끝나면 Capsomnia를 끕니다. 외부 제어기 호환 모드가 꺼져 있을 때만 Mac을 즉시 잠자기 상태로 전환합니다.
- Caps Lock 끄기: Capsomnia의 잠자기 방지를 한 번 해제합니다. 외부 제어기 호환 모드에서는 이후 다른 앱이 다시 켠 상태를 덮어쓰지 않습니다.
- Capsomnia가 켜진 상태에서 덮개 닫기: 외부 디스플레이가 연결되어 있지 않을 때만 디스플레이를 끄고 작업은 계속 돌립니다.
- 앱 종료: Capsomnia가 켜져 있으면 잠자기 방지를 해제합니다. 호환 모드에서 이미 꺼져 있으면 외부 제어기의 상태를 바꾸지 않습니다.
...
- 호환 모드가 꺼져 있으면 자동 종료 타이머는 시간이 끝날 때 Mac을 명시적으로 잠자기 상태로 전환합니다. 작업을 저장하고 충분한 시간을 설정하세요.
...
‘덮개를 닫을 때 화면 끄기’, ‘외부 잠자기 제어 존중’, ‘로그인할 때 열기’는 기본적으로 켜져 있으며 초기 설정에는 표시되지 않습니다. 초기 설정 이후 Capsomnia를 열면 고급 설정이 바로 열리고, 뒤로 가기로 간단한 설정 페이지로 돌아갈 수 있습니다. 호환 모드는 Capsomnia를 끌 때 `SleepDisabled=0`을 한 번만 쓰고, 이후 외부에서 쓴 `1`은 다른 제어기가 소유한 것으로 받아들입니다. 메뉴 막대는 회색 꺼짐 상태를 유지하며 툴팁으로 외부 상태를 설명합니다. 고급 설정에서는 자동 종료 타이머와 전역 단축키도 지정할 수 있습니다. 타이머는 기본적으로 꺼져 있습니다. Capsomnia를 켤 때마다 지속 시간 타이머는 선택한 시간 전체로 다시 시작하고, 시각 지정은 다음 해당 시각으로 예약됩니다. 재시작 버튼으로 현재 카운트다운을 처음부터 다시 셀 수 있습니다. ‘대문자 고정 방지’를 활성화해도 ‘메뉴 막대에 표시’는 별도로 켜거나 끌 수 있습니다. 메뉴 막대 아이콘을 숨겨 두었더라도 오류가 발생하면 빨간 점이 잠시 나타납니다.
...
패키지로 설치한 앱 파일, helper, 시스템 LaunchAgent의 소유자는 `root:wheel`입니다. 패키지의 helper도 앱과 같은 Developer ID로 서명합니다. Capsomnia는 설정을 바꿀 때마다 실제 `SleepDisabled` 상태를 확인하고 이후에도 10초마다 점검합니다. helper가 변경을 적용하지 못하거나 상태를 확인할 수 없거나 설정이 어긋나면 메뉴 막대의 점을 빨간색으로 바꾸고 5초 뒤 다시 시도합니다. 단, 외부 제어기 호환 모드가 켜져 있고 Capsomnia가 꺼진 상태에서 `SleepDisabled=1`이면 외부 값으로 받아들이고 다시 0으로 쓰지 않습니다. 평소 메뉴 막대 아이콘을 숨겨 두었더라도 오류가 발생하면 빨간 점이 잠시 나타납니다.
...
외부 제어기 호환 모드가 꺼져 있으면 자동 종료 타이머는 Caps Lock을 정상적으로 끄고 `SleepDisabled=0`을 확인한 뒤 현재 사용자 권한으로 `/usr/bin/pmset sleepnow`를 직접 실행합니다. 호환 모드에서는 이 즉시 잠자기 요청을 취소합니다. 이 명령은 `sudo`를 사용하지 않으며 helper나 sudoers 권한을 늘리지 않습니다.
